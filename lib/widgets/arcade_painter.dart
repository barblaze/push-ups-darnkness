...
class ArcadePainter extends CustomPainter {
  ...
  void _drawSky(Canvas canvas, double w, double h) {
    ...
    final top = Color.lerp(const Color(0xFF131A33), const Color(0xFF2A1A45), shift / 24)!;
    final mid = Color.lerp(const Color(0xFF4A2460), const Color(0xFF3D3D6E), shift / 24)!;
    final bottom = Color.lerp(const Color(0xFF7A2F52), const Color(0xFF5C2A6B), shift / 24)!;
  ...
  void _drawStars(Canvas canvas, double w, double h) {
    ...
    for (final star in _starPositions) {
      final alpha = 0.25 + 0.25 * math.sin(game.elapsed * star.tw + star.x * 40);
  ...
  void _drawHills(Canvas canvas, double w, double h) {
    ...
    for (var layer = 0; layer < 2; layer++) {
      ...
      canvas.drawPath(
        path,
        Paint()..color = const Color(0xFF1C1440).withValues(alpha: 0.45 + layer * 0.2),
      );
    }
  }
  ...
  void _drawClouds(Canvas canvas, double w, double h) {
    ...
    for (final layer in layers) {
      ...
      final offset =
          (game.elapsed * game.speed * layer.speed * w) % (w + 200);
      for (var i = 0; i < 3; i++) {
        final x = w - offset + i * w * 0.62;
        if (x < -220 || x > w + 220) continue;
        _drawCloud(canvas, Offset(x, h * layer.y), 90 * layer.scale, 26 * layer.scale, paint);
      }
    }
  }

  void _drawCloud(Canvas canvas, Offset c, double width, double height, Paint paint) {
    canvas.drawOval(Rect.fromCenter(center: c, width: width, height: height), paint);
    canvas.drawOval(
      Rect.fromCenter(
  ...
  void _drawObstacles(Canvas canvas, double w, double h) {
    ...
    for (final obstacle in game.obstacles) {
      ...
      final topRRect = RRect.fromRectAndCorners(top, bottomRight: const Radius.circular(12));
      canvas.drawRRect(topRRect, fill);
      canvas.drawRRect(topRRect, dark..style = PaintingStyle.stroke..strokeWidth = 2);
      ...
      canvas.drawRRect(bottomRRect, dark..style = PaintingStyle.stroke..strokeWidth = 2);
      ...
      final capPaint = Paint()
        ...
        ).createShader(Rect.fromLTWH(cx - pillarW * 0.62, 0, pillarW * 1.24, h));
  ...
  void _drawSpeedLines(Canvas canvas, double w, double h) {
    ...
    final paint = Paint()
      ...
      ..strokeCap = StrokeCap.round;
    for (var i = 0; i < 8; i++) {
      final offset = (game.elapsed * game.speed * w * (1.2 + (i % 3) * 0.4)) % (w + 300);
      final x = w - offset;
  ...
  void _drawCharacter(Canvas canvas, double w, double h) {
    if (game.invulnerable && !game.gameOver && (game.elapsed * 6).floor().isEven) {
      return;
    }
  ...
  void _drawWings(Canvas canvas, double r) {
    final flap = game.gameOver
        ? math.pi / 2
        : math.sin(game.elapsed * 15) * 0.65;
  ...
  void _drawBody(Canvas canvas, double r) {
    ...
    final suit = Paint()
      ..shader = RadialGradient(
        colors: [Color.lerp(characterColor, Colors.white, 0.25)!, characterColor],
    ...
    // Cuerpo.
    canvas.drawOval(
      Rect.fromCenter(center: Offset(0, -r * 0.05), width: r * 1.3, height: r * 1.35),
      suit,
    );

    // Cabello.
    final hair = Paint()..color = Color.lerp(characterColor, Colors.black, 0.2)!;
    canvas.drawArc(
    ...
    if (game.gameOver) {
      ...
      final mouth = Paint()
        ...
        ..style = PaintingStyle.stroke
        ..strokeWidth = r * 0.09;
      canvas.drawCircle(Offset(0, -r * 0.62), r * 0.16, mouth..style = PaintingStyle.stroke);
    } else {
      ...
      if (blink) {
        ...
        for (final side in const [-1.0, 1.0]) {
          canvas.drawCircle(Offset(side * r * 0.24, -r * 0.9), r * 0.16, white);
          canvas.drawCircle(Offset(side * r * 0.24, -r * 0.88), r * 0.08, pupil);
        }
      }
      final smile = Paint()
        ..color = const Color(0xFF5A3A2A)
  ...
  void _drawDeathFeathers(Canvas canvas, double r) {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      final x = math.cos(a) * (0.15 + t * 0.9) * r * 2.4 + math.sin(a * 2) * r * 0.4;
      final y = math.sin(a) * 0.15 * r * 2 + t * (speed) * r * 2.2 + t * t * r * 2;
      final alpha = (1 - t / 1.2).clamp(0.0, 1.0);
      canvas.save();
      canvas.translate(x, y);
      canvas.rotate(t * 3 + i);
      paint.color = Colors.white.withValues(alpha: alpha * 0.9);
      canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: r * 0.34, height: r * 0.16), paint);
      canvas.restore();
    }
  }
